...
import { addContactToList } from "../Actions/ContactActions";
import Navbar from "./Navbar";
import { useParams } from "react-router-dom";
import * as XLSX from "xlsx";

function ImportContact({ addContactToList, lists }) {
  const { id } = useParams();
  ...
  const handleUpload = async () => {
    ...
    reader.onload = (e) => {
      ...
      for (let row of jsonData) {
        // Assuming the row has columns: name, lastName, email, phone
        const { name, lastName, email, phone } = row;
        console.log("Row data:", row);
        // Dispatch action to add contact to the list
        addContactToList(id, { name, lastName, email, phone });
      }
  ...
  const handleCopyPaste = () => {
    ...
    const contacts = lines.map((line) => {
      const [name, lastName, email, phone] = line.split("\t");
    ...
    contacts.forEach((contact) => {
      ...
      if (contact.name && contact.email && contact.phone) {
        // Dispatch addContactToList action
        addContactToList(id, contact);
      }
    });
  ...
  const handleFormSubmit = (e) => {
    ...
    // Dispatch addContactToList action with form data and listId
    addContactToList(id, formData);
...
export default connect(mapStateToProps, { addContactToList })(ImportContact);
